...
class OracleOrmParser {
	...
	function connect() {
		...
		{
			$this->connect = oci_pconnect (	$this->params['user'],
											$this->params['password'],
											$this->params['host'] . '/' . $this->params['name'], 
											$this->options['charset']);
		} else {
			$this->connect = oci_connect (	$this->params['user'],
											$this->params['password'],
											$this->params['host'] . '/' . $this->params['name'], 
											$this->options['charset']);
		}
		...
		switch ($a['mode']) {
			case 'select':
				$sql = 'SELECT ' . $this->parseSqlFieldList($a['field'], true)
						. ' FROM ' . $this->parseSqlTable($a['table'], false);
			...
			case 'update':
				...
				break;
			case 'insert':
				$sql = 'INSERT INTO ' . $this->parseSqlTable($a['table'], false)
	...
	function encodeFieldValue($value, $type) {
		if (is_array($value)) {
			...
			switch ($type) {
				...
				case _ORM_DT_SQL: $result = $value; break;
	...
	function fetch($dataset, $fetchMode = _ORM_FETCH_ASSOC) {
		switch ($fetchMode) {
			case _ORM_FETCH_ASSOC: 
				$result = oci_fetch_assoc($dataset); break;
			case _ORM_FETCH_NUM: 
				$result = oci_fetch_row($dataset); break;
			case _ORM_FETCH_BOTH: 
				$result = oci_fetch_array($dataset); break;
			case _ORM_FETCH_OBJ: 
				$result = oci_fetch_object($dataset); break;
		}
	...
	function getAll($sql, $offset=0, $count=-1, $fetchMode = _ORM_FETCH_ASSOC) {
		$result = array();
		$dataset = $this->execute($sql);
		while ($row = oci_fetch_array($dataset, $fetchMode)) {
			$result[] = $row;
		}
		oci_free_statement($dataset);
		return $result;
	...
	function execute($sql) {
		if (OracleOrmParser::$activeParams !== $this->params)
		{
			OracleOrmParser::$activeParams = $this->params;
	...
	function getAffectRows() {
		return oci_num_rows ($this->connect);
	}	
